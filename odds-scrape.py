...
def detect_changes(new_data, prev_data):
        ...
        for idx in changes.index:
            ...
            if "line" in changes.columns.levels[0]:
                line_new = changes.at[idx, ("line", "self")] if ("line", "self") in changes.columns else None
                line_old = changes.at[idx, ("line", "other")] if ("line", "other") in changes.columns else None
            else:
                line_new, line_old = None, None

            if "price" in changes.columns.levels[0]:
                price_new = changes.at[idx, ("price", "self")] if ("price", "self") in changes.columns else None
                price_old = changes.at[idx, ("price", "other")] if ("price", "other") in changes.columns else None
            else:
            ...
            # Moneyline changes
                ...
                output.append(
                    f"{team} moneyline moved from {price_old} to {price_new}"
                )

            # Totals changes
            elif bet_type == "Total" and game not in processed_games and (line_new is not None):
                processed_games.add(game)
                try:
                    over_new = new_data[(new_data["game"] == game) & (new_data["team"] == "Over")]["price"].iloc[0]
                    under_new = new_data[(new_data["game"] == game) & (new_data["team"] == "Under")]["price"].iloc[0]

                    over_old = prev_data[(prev_data["game"] == game) & (prev_data["team"] == "Over")]["price"].iloc[0]
                    under_old = prev_data[(prev_data["game"] == game) & (prev_data["team"] == "Under")]["price"].iloc[0]
