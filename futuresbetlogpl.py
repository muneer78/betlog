...
df = df.with_columns(
    pl.when((df['Result'] == 'L') & (df['FreeBet'] == 'N')).then(df['Amount'] * -1)
    .when((df['Result'] == 'L') & (df['FreeBet'] == 'Y')).then(0)
    .when((df['Result'] == 'W') & (df['FreeBet'] == 'Y')).then(df['PotentialProfit'])
    .when((df['Result'] == 'W') & (df['FreeBet'] == 'N')).then(df['PotentialPayout'])
    .when(df['Result'] == 'Pe').then(0)
    .when(df['Result'] == 'P').then(df['PushAmount'])
...
filter2 = filter2.with_columns(
    pl.when(filter2['Result'] == 'P').then(filter2['PushAmount'])
