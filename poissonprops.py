...
data = [
    {"Player": "Damian Lillard", "Team": "Blazers", "Prop Stat": "Rebounds", "Which side is being offered in prop?": "Under", "Which side to bet?": "Over", "Odds offered": 125, "Prop": 3.5, "Average": 4, "Probability": None, "Break Even Odds": None, "No Hold Probability": "56%", "No Hold Line": -125, "Does Over/Under Match?": False, "Action to take": "No Bet"},
    {"Player": "Jerami Grant", "Team": "Blazers", "Prop Stat": "Assists", "Which side is being offered in prop?": "Over", "Which side to bet?": "Over", "Odds offered": 120, "Prop": 2.5, "Average": 2.7, "Probability": None, "Break Even Odds": None, "No Hold Probability": "55%", "No Hold Line": -120, "Does Over/Under Match?": False, "Action to take": "Bet"},
...
# Function to compare "Which side is being offered in prop?" and "Which side to bet?" columns
...
def calculate_break_even_odds(row):
    if pd.notna(row['Probability']):
        ...
        return int((-100 if probability > 0.5 else 100) * probability / (1 - probability))
    return None
...
def calculate_no_hold_line(row):
    if pd.notna(row['No Hold Probability']):
        no_hold_probability = float(row['No Hold Probability'].strip('%')) / 100
        return int((-100 if no_hold_probability > 0.5 else 100) * no_hold_probability / (1 - no_hold_probability))
    return None

# Function to calculate Action to Take
def calculate_action_to_take(row):
    if pd.notna(row['Odds offered']) and pd.notna(row['No Hold Line']) and row['Does Over/Under Match?']:
...
# Display the DataFrame with updated "Probability" and "Break Even Odds" columns
print(df)
