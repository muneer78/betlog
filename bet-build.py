...
df["net_profit"] = df["gross_profit"] - df.groupby("bet_group")["Amount"].transform(
    "sum"
)
...
results = (
    df.groupby("bet_group")
    .agg({"ROI": ["min", "mean"], "ActualPayout": "sum"})
    .reset_index()
)
...
with open(csv_filename, "w", newline="") as file:
    for group_name, group_data in results.iterrows():
        ...
        if "P" in group_df["Result"].values:
            minimum_roi = (
                round(group_data["Minimum ROI"], 2)
                if "Minimum ROI" in group_data
                else "N/A"
            )
            average_roi = (
                round(group_data["Average ROI"], 2)
                if "Average ROI" in group_data
                else "N/A"
            )
            ...
            total_payout = (
                format_currency(group_data["Total Payout"])
                if "Total Payout" in group_data
                else "N/A"
            )
