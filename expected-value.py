...
winnings = round(((cleaned_odds / 100) * amount) if odds >
                 0 else ((100 / cleaned_odds) * amount), 2)
win_str = str(winnings)

implied_probability = round((100 / (100 + cleaned_odds))
                            if odds > 0 else (cleaned_odds / (100 + cleaned_odds)), 2)
implied_probability_100 = implied_probability * 100
implied_probability_str = str(implied_probability_100)

ev = round((implied_probability * winnings) -
           ((1 - implied_probability) * amount), 2)
str_ev = str(ev)
